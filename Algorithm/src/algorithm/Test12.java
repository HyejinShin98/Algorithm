...
class Solution12 {
    public String solution(int[] food) {
        ...
        // food[0] : 1은 물이므로 food[1] 부터
        for(int i=1; i<food.length; i++) {
        	
        	// 음식의 수를 2로 나눠서 소수점 버림
        	int foodCntByPerSon = (int)(Math.floor(food[i] / 2));
        	for(int j=0; j<foodCntByPerSon; j++) {
        		answer.append(i);
        	}
        }
        
        String addStr = new StringBuilder(answer).reverse().toString();
        answer.append(0).append(addStr);
